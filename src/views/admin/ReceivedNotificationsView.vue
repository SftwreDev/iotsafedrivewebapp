...
fetchAcceptedAccidents()
fetchRejectedAccidents()
</script>
...
<template>
            ...
            <div class="relative flex justify-between lg:gap-8 xl:grid xl:grid-cols-12">
                <div class="flex md:absolute md:inset-y-0 md:left-0 lg:static xl:col-span-2">
                    <div class="flex flex-shrink-0 items-center">
                        <p v-if="lastSegmentPath === 'received-accidents'" class="text-xl font-medium text-gray-800 ">
                            Received Accidents
                            <br>
                            <span class="text-xs text-gray-500">Reports for received accidents from the command center
                                (accepted and rejected)</span>
                        </p>

                        <p v-if="lastSegmentPath === 'forwarded-accidents'" class="text-xl font-medium text-gray-800 ">
                            Forwarded Accidents
                            <br>
                            <span class="text-xs text-gray-500">Reports for forwarded accidents from the command center
                                (accepted and rejected)</span>
                        </p>
                    </div>
                </div>
        ...
        <div class="hidden sm:block">
            <div class="border-b border-gray-200">
                <nav aria-label="Tabs" class="-mb-px flex space-x-8">
                    <p v-for="tab in tabs" :key="tab.name" :aria-current="tab.current ? 'page' : undefined"
                       :class="[currentTab === tab.tab_name ? 'border-indigo-500 text-indigo-600' : 'cursor-pointer border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700', 'whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium']"
                       @click="currentTab = tab.tab_name">{{ tab.name }}</p>
                </nav>
            </div>
        </div>
    </div>
    ...
    <div class="mx-auto max-w-9xl px-4 sm:px-6 lg:px-8">
        <div v-if="currentTab === 'accepted'">
            <div v-if="!loading">
                <CustomDataTable :cols="acceptedAccidentsColumns" :isLoading="loading" :rows="accepted_accidents"
                />
            </div>
        </div>
        <div v-if="currentTab === 'rejected'">
            <div v-if="!loading">
                <CustomDataTable :cols="rejectedAccidentsColumns" :isLoading="loading" :rows="rejected_accidents"
                />
            </div>
        </div>
    </div>
